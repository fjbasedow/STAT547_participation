...
a <- c(1,2,3)
b <- c(4,5,6)
map2(a, b, function(x, y) x*y) # function has to be funciton of two iterables, pairwise applies x,y function
```
```{r}
map2(a, b, ~ .x * .y)
```
...
```{r}
map2_chr(name, commute, ~ str_c(.x, " takes ", .y, " minutes to get to work.")) # ..1 and ..2 works too
```
...
(listcol_tib <- tibble(
  ...
  d = list(x=1, y=sum, z=iris) # assign names to list columns
))
```
...
got_alias <- got_chars %>% {
  tibble(name = map_chr(., "name"),
         aliases = map(., "aliases")) #map_chr need character vector of length 1
}
got_alias
```
...
```{r}
got_alias %>% 
  {setNames(.$aliases, .$name)} %>% # names arguments of lsit. first argument: list to name, second: names
  `[[`("Melisandre")
```
...
```{r}
icecream %>% 
  filter(map_lgl(flav, ~ any(.x == "strawberry"))) # map logical
```
