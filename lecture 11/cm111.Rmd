...
html_text(simple_page) %>% cat() # cat is base R function. Renders /n
```
...
```{r}
simple_page %>% 
	html_nodes("a") %>% html_attr(name="href")
```
...
```{r}
read_html("https://www.musixmatch.com/search/kane%20brown#") %>% html_nodes(".track-card .media-card-title") %>% html_text()
```
