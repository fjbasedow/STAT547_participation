...
```{r}
str_split(fox, pattern = " ")#splitting up fox sentence by space. Output is a list
...
```{r}
fox %>% 
  str_split(pattern = "") %>% # splits into individual characters/letters
  `[[`(1) %>% #removing overarching list structure, subsetting content of first entry in list
  str_to_lower %>% 
  table()
```
...
fruit %>% head
fruit %>% 
    str_pad(width=7, side="right", pad="$") %>% #every word at least width 7, if not add $ to the right to reach minimum of 7 letters
    head()
```
...
```{r}
str_length(fruit) # tells how many characters there are in each entry
length(fruit) # how many entries in vector
```
...
```{r}
# similar to paste function
str_c(words[1:4], words[5:8], sep=" & ")
str_c(words[3:4], words[5:8], sep=" & ") # recycling vector of length 2
str_c(words[3:4], words[5:8], sep=" & ", collapse=", ") #binds everything that was combined together, output is vector of length 1
```
...
```{r}
(num <- str_pad(3:12, width = 2, side = "left", pad = " "))
```
...
```{r}
for (i in 1:length(folders)) dir.create(here::here(folders[i]))
```
...
```{r}
(files <- str_c(here::here(folders), "/README.md"))
```
...
```{r}
str_subset(countries, pattern = "i.a") # returns all countries that have "i something a" in there name
```
...
```{r}
str_view_all_match(countries, pattern = "[^aeiou][^aeiou][^aeiou]") #3 non-vowels next to each other
```
...
- `*` for 0 or more of preceding character
- `+` for 1 or more
- `?` for 0 or 1
...
```{r}
str_view_all_match(countries, "ro+") # countries with r's that are followed by o at least once
```
...
```{r}
str_view_all_match(countries, "e{2}") # countries that have 2 e's 
```
...
```{r}
str_view_all_match(countries, pattern = "land$") #all countries that end in "land"
str_view_all_match(countries, pattern = "$") # finds end of each country
```
...
```{r}
str_view_all_match(countries, "^[^aeiouAEIOU]*$") #no countries that doesn't contain vowels at all
```
...
```{r}
str_view_all(c("abad", "abbd"), pattern="(a)(b)\\1") # look for a followed by b. a is group 1, b is group 2 (defined by brackets), //1 = whatever was resolved in group 1
...
```{r}
str_view_all(c("bananas"), "(.)(.)\\1\\2.*\\1\\2") #anything followed by anything, repeat the first group, repeat the second group, followed by any number of anything (could be many)
```
